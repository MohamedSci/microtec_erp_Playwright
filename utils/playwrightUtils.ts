...
async function displayingInvalidEmailMessage(page: Page) {
...
async function displayingRequiredMessage(page: Page) {
    const spanLocator = page.locator('span');
    await expect(spanLocator).toContainText(/required/i);
}


async function increaseScreenItemsCountToHundred(page: Page) {
...
async function navigateToTheLatestScreen(page: Page) {
...
async function scrollToElement(page: Page, containerSelector: string, elementSelector: string) {
...
async function scrollToLastElement(page: Page) {
...
async function verifyALastCellInTable(page: Page, col: number, txt: string) {
    const tableRows = await page.$$('table tbody tr');
    const lastRow = tableRows[tableRows.length - 1];
    const cells = await lastRow.$$('td');
...
async function inputText(page: Page, index: number, str: string) {
    const input = await page.$$('input[type="text"]');
...
async function visibilityOfRequiredStar(page: Page, fieldSelector: string) {
...
async function validateRequiredComponents(page: Page, len: number) {
...
async function VerifyTableVisibility(page: Page, my_table: string) {
...
async function getAllItemsCount(page: Page, gridSelector: string, itemSelector: string) {
...
async function getLoginResponse(page: Page): Promise<LoginResponse | null> {
